...
app.post('/whatsapp', async (req, res) => {
  ...
  if (incomingMessage === 'hello') {
    responseMessage = 'Hello! Would you like to set a reminder or exit? Please reply with "set reminder" or "exit".';
  }else if(incomingMessage === 'set reminder'){
    responseMessage = 'Please enter the time for the reminder (in 24-hour format or 12-hour format with AM/PM).';
    ...
    if (!reminderTime.isValid()) {
      responseMessage = 'Invalid time format. Please enter the time in 24-hour format (e.g., "14:00") or 12-hour format with AM/PM (e.g., "2:00 PM").';
    } else {
      reminders[from] = {
    ...
    await db.query('INSERT INTO reminders (phone_number, reminder_time, message) VALUES ($1, $2, $3)', [phoneNumber, reminderTime, reminderMessage]);
    ...
    responseMessage = 'Sorry, I did not understand that. Please reply with "hi" to start over.';
  }
...
cron.schedule('* * * * *', async () => {
  ...
  const reminders = await db.query('SELECT * FROM reminders WHERE reminder_time <= $1', [now]);
